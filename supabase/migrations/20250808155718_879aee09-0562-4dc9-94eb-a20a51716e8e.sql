-- Update function: link organization_members to existing auth.users only; do not create public.users
...
BEGIN
  ...
  -- Link membership only if an auth user with the email exists
  IF nullif(v_contact_email,'') IS NOT NULL THEN
    -- read from auth.users (do not create)
    SELECT u.id INTO v_user_id FROM auth.users u WHERE u.email = v_contact_email LIMIT 1;
    ...
    IF FOUND THEN
      ...
      -- Ensure a basic app role if table exists
      IF EXISTS (
        ...
        END IF;
      END IF;
    END IF; -- FOUND auth user
  END IF; -- email present

  RETURN v_org_id;
END; $function$;

-- Re-run backfill safely
DO $$
DECLARE
  r record;
  v_name text;
