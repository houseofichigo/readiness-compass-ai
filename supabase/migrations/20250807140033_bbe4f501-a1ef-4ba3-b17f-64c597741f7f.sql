-- Fix populate_answer_metadata trigger to work with new question structure
...
-- Create trigger for answer metadata population (if not exists)
DROP TRIGGER IF EXISTS populate_answer_metadata_trigger ON public.answers;
CREATE TRIGGER populate_answer_metadata_trigger
    BEFORE INSERT OR UPDATE ON public.answers
    FOR EACH ROW
    EXECUTE FUNCTION public.populate_answer_metadata();

-- Backfill existing answers with proper metadata
WITH question_data AS (
    SELECT 
        ...
        helper as question_helper,
        options as question_options,
        required as question_required,
        score_map_by_bucket,
        category,
        purpose,
        pillar_scores,
...
SET 
    ...
    question_helper = qd.question_helper,
    question_options = qd.question_options,
    question_required = qd.question_required,
    category = qd.category,
    purpose = qd.purpose,
    pillar_scores = qd.pillar_scores,
    pillar_options = qd.pillar_options,
    pillar_logic = qd.pillar_logic,
    score_map_by_bucket = qd.score_map_by_bucket
FROM question_data qd
WHERE a.question_id = qd.question_id;

-- Function to backfill option metadata for existing answers
