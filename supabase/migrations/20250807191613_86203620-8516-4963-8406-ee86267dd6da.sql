-- Fix seed_assessment to guard array handling and avoid scalar extraction errors
  ...
  for s in select jsonb_array_elements(_sections) loop
    ...
    for q in select jsonb_array_elements(coalesce(s->'questions', '[]'::jsonb)) loop
      ...
      values (
        ...
        case 
          when q ? 'weight' and jsonb_typeof(q->'weight') = 'array' then (
            select array_agg((e)::int)
            from jsonb_array_elements_text(q->'weight') as e
          )
          else null
        end,
