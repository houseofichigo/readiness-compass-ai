-- Fix organizations table security issue - clean slate approach
...
-- Drop the basic view if it exists (from previous failed migration)
DROP VIEW IF EXISTS public.organizations_basic;
...
-- Policy 1: Only owners/managers can see their full organization data (including sensitive info like contact emails, revenue, etc.)
...
-- This will be enforced by using a security definer function for safe access
...
USING (
  ...
  -- Application code must use get_org_basic_info() function for regular members
);

-- Create a security definer function that returns safe organization data for regular members
-- This excludes sensitive fields like primary_contact_email, annual_revenue_exact, linkedin_url, verification_notes, etc.
